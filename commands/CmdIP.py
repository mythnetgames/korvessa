...
class CmdInvest(Command):
    ...
    def _show_ip_overview(self, caller, current_ip):
        ...
        msg.append("")
        msg.append("|cSkill Tiers & Costs:|n")
        msg.append("  |w0-20|n (Novice):       1 IP base, +4% per level")
        msg.append("  |w21-45|n (Competent):   2 IP base, +6% per level")
        msg.append("  |w46-75|n (Professional): 4 IP base, +7.5% per level")
        msg.append("  |w76-89|n (Advanced):    8 IP base, +9% per level")
        msg.append("  |w90-99|n (Mastery):     16 IP base, +12% per level")
        msg.append("")
        msg.append("|cSample Costs:|n")
        msg.append(f"  0->1: {ip_cost_for_next_point(0)} IP   |  50->51: {ip_cost_for_next_point(50)} IP")
        msg.append(f"  20->21: {ip_cost_for_next_point(20)} IP  |  75->76: {ip_cost_for_next_point(75)} IP")
        msg.append(f"  45->46: {ip_cost_for_next_point(45)} IP  |  99->100: {ip_cost_for_next_point(99)} IP")
        msg.append("")
        msg.append(f"|cTotal IP to reach milestones (from 0):|n")
        msg.append(f"  Level 20: {total_ip_to_reach(20)} IP  |  Level 50: {total_ip_to_reach(50)} IP")
        msg.append(f"  Level 75: {total_ip_to_reach(75)} IP  |  Level 100: {total_ip_to_reach(100)} IP")
        msg.append("")
